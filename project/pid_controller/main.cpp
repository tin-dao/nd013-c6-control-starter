...
// Use arguments to tune PID values
int main (int argc, char **argv)
{
  ...
  if (argc > 6) {
    ...
    steer_I = std::stod(argv[2]);
    steer_D = std::stod(argv[3]);
    throttle_P = std::stod(argv[4]);
    throttle_I = std::stod(argv[5]);
    throttle_D = std::stod(argv[6]);
  }
